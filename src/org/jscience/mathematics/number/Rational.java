...
public final class Rational extends Number<Rational> implements Field<Rational>{
    ...
    /**
     * Returns the rational number for the specified double
     *
     * @param number the double value.
     * @return rational interpretation of <code>number</code>
     */
    public static Rational valueOf(Float64 number) {
        double value = number.doubleValue();
        Rational r = FACTORY.object();
        if (value == 0) {
            r._dividend = LargeInteger.ZERO;
            r._divisor = LargeInteger.ONE;
        } else {
            ...
            LargeInteger dividend = LargeInteger.valueOf(a);
            LargeInteger divisor = LargeInteger.valueOf(b);

            if (exponent > 0)
                dividend = dividend.times2pow((int) exponent);
            else
                divisor = divisor.times2pow((int) -exponent);

            if (sign == 1)
                dividend = dividend.opposite();
